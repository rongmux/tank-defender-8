...
assert(schema.gameSettings.explosionRules.baseDestroy.ttl === 35, "base destruction should expose the original thirty-five visible explosion frames");
...
assert(schema.gameSettings.explosionRules.playerDestroy.ttl === 18, "player destruction should expose the original eighteen explosion-state ticks");
...
assert(playerGameOverMessageProbe.p1.initial.timer === 13 && playerGameOverMessageProbe.p1.initial.x === 0x20 && playerGameOverMessageProbe.p1.initial.y === 0xd8 && playerGameOverMessageProbe.p1.initial.dx === 1 && playerGameOverMessageProbe.p1.initial.tickLow === 0, "player one elimination should reset the low frame phase and initialize the message at the original left-side coordinates");
assert(playerGameOverMessageProbe.p2.initial.timer === 13 && playerGameOverMessageProbe.p2.initial.x === 0xc0 && playerGameOverMessageProbe.p2.initial.y === 0xd8 && playerGameOverMessageProbe.p2.initial.dx === -1 && playerGameOverMessageProbe.p2.initial.tickLow === 0, "player two elimination should initialize the mirrored right-side message");
...
assert(playerGameOverMessageProbe.commonGameOver.screen === "gameOver" && playerGameOverMessageProbe.commonGameOver.message === null, "the shared game-over route should replace any individual player message");
canvasContext.calls.length = 0;
