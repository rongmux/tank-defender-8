...
- Six power-ups with distinct original-style 16x16 replacement icons: grenade, helmet, shovel, star, timer, and extra tank. Random releases use the original eight-entry lookup, giving grenade and star two chances each and the other four items one chance each. Active players collect an item only when both coordinate differences are below 12 pixels; spawning players cannot collect it. If both players qualify on the same frame, player 2 is checked first. The shovel changes only the five base-wall cells from brick to steel, flashes near expiry, and restores the wall afterward; it never creates ice terrain.
...
- Power-up pickup score, extra-life score thresholds, and grenade clearing without enemy kill-table credit. Stage completion is based on the full enemy queue being spawned and all active enemies being gone; kill-table credit does not gate advancement.
- In-field score popups for scored enemy kills and collected power-ups. Grenade-cleared enemies still omit enemy score popups because they do not receive kill-table score credit.
