...
The architecture is split by function: `src/game.js` is the application composition root, pure domain modules own rules and data, and focused runtime modules own browser effects and orchestration. Tests are split into unit suites, feature integration suites, and shared harnesses. The app keeps its static, no-build launch path and does not require a package install.

The structural refactor is complete. The next phase is reference-driven 1:1 fidelity verification, not further splitting by file size. Passing the existing regression suite protects current behavior; it does not by itself prove equivalence to the NES game. The architecture and verification boundaries are described below.
...
Node.js is needed only for automated verification. No package installation is required.
...
The runner includes the browser-entry script-order check, public API and probe hash regressions, and bilingual file-tree validation. For a focused check, run the relevant `tests/unit/*.test.js` or `tests/integration/*.test.js` file directly with Node.
...
|   |   |-- screen-flow-post-game-diagnostics.js
...
|   |   |-- audio-movement-runtime.test.js
|   |   |-- audio-voice-runtime.test.js
|   |   |-- application-flow-composition-runtime.test.js
|   |   |-- input-composition-runtime.test.js
|   |   |-- legacy-api-composition-runtime.test.js
|   |   |-- render-pipeline-composition-runtime.test.js
...
|   |   |-- public-api-adapters.test.js
|   |   |-- effect-presentation.test.js
|   |   |-- enemy-spawn-diagnostics.test.js
|   |   |-- enemy-diagnostics.test.js
|   |   |-- enemy-ai-rules.test.js
|   |   |-- enemy-ai-settings.test.js
|   |   |-- enemy-sequences.test.js
...
|   |   |-- power-up-collection-diagnostics.test.js
...
|   |   |-- screen-flow-post-game-diagnostics.test.js
|   |   |-- screen-flow-diagnostics.test.js
|   |   |-- screen-presentation.test.js
|   |   |-- stage-flow-transition-diagnostics.test.js
|   |   |-- stage-flow-progression-diagnostics.test.js
|   |   |-- stage-flow-game-over-diagnostics.test.js
|   |   |-- stage-flow-diagnostics.test.js
|   |   |-- stage-flow-settings.test.js
|   |   |-- stage-settings.test.js
|   |   |-- stage-grid.test.js
...
| --- | --- | --- |
| Core | `src/core/` | Fixed-frame counters, random arithmetic, directions, and geometry. |
| Configuration | `src/config/` | Validated defaults and normalization for timing, combat, enemies, movement, upgrades, power-ups, and stage rules. |
| Stages | `src/stages/` | Built-in maps and enemy sequences, grid geometry, pack parsing/schema, routing, and active-pack lookups. |
| Entities | `src/entities/` | Player, enemy, projectile, collectible, explosion, and score-popup records and lifecycle transitions. |
| Gameplay rules | `src/rules/` | AI decisions, spawning, collision and overlap recovery, wall damage, power-up selection/effects, scoring, and results. |
| Editor | `src/editor/` | Construction patterns, cursor/brush rules, quadrant mutations, save formats, and pack conversion. |
| Presentation | `src/presentation/` | Replacement sprites, pixel fonts, layout geometry, palettes, and visual timelines without Canvas side effects. |
| Audio | `src/audio/` | Replacement sound data, fixed-frame sound state, voice projection, and channel priorities without Web Audio nodes. |
| Runtime | `src/runtime/` | State ownership, composition, browser input/storage, Web Audio playback, Canvas drawing, fixed-frame updates, and diagnostics. |
| Data and tools | `data/`, `tools/` | Replacement manifests, example pack data, pack generation, and the optional loopback development server. |
| Tests | `tests/unit/`, `tests/integration/`, `tests/helpers/` | Domain-level checks, production-module integration, deterministic browser doubles, and test discovery. |

### Runtime Composition

`index.html` loads the explicit browser modules before `src/game.js`. Modules register in `window.TankDefender8Modules`; domain modules also expose CommonJS exports for direct Node tests. No bundler or runtime dependency download is involved.

1. `shared-state.js` creates the mutable game/input/audio state. `module-deps.js` resolves registered modules and exposes the dependency barrel.
2. The entry creates the active `stage-runtime.js` adapter and the deterministic `debug-battle-runtime.js` fixture.
3. `application-flow-composition-runtime.js` installs lifecycle, title/editor/stage flow, audio, and related screen services.
4. `render-pipeline-composition-runtime.js` prepares text, sprite, and render adapters. `render-composition-runtime.js` installs the specialized renderers during pipeline completion.
5. `battle-systems-composition-runtime.js` installs movement, combat, timers, scoring, power-ups, enemies, and projectiles in the required order. `battle-composition-runtime.js` adds battle, frame, and screen-update loops.
6. After the render pipeline finishes, `input-composition-runtime.js` wires keyboard/mouse/toolbar/file events; `legacy-api-composition-runtime.js` and `debug-api.js` expose the compatibility and diagnostic surfaces.
7. The entry loads the high score, creates the initial grid, and starts `frame-loop-runtime.js`.

`state.fn` is the shared registry of installed runtime callbacks, not a second copy of the game state. Composition modules pass explicit callbacks to their children; `stage-runtime.js` reads the current pack dynamically. Grid-domain constants such as `QUAD_GRID` come from `stage-grid.js` through the dependency barrel, while screen geometry and shared state factories come from `shared-state.js`.

`frame-loop-runtime.js` advances simulation at a fixed 60 Hz using an accumulator, renders on each animation frame, and clamps long gaps to 80 ms. Browser refresh rate is not the simulation rate. `screen-update-runtime.js` dispatches by screen; `battle-loop-runtime.js` owns battle update order. Rule, input, audio, and render modules must not add independent gameplay clocks.

### Diagnostics And Tests

`diagnostic-scope.js` centralizes receiver-preserving callback binding. Callback precedence remains `state.fn > state.stageRuntime > deps`; shared values overlay raw dependency values, and game/input/audio records stay live references. Probe modules own setup, results, and restoration. `debug-api.js` only composes the public surface; `public-api-adapters.js` owns pack loading, metadata, snapshots, and schema projections.

The complete 159-method public API order is locked by `tests/integration/public-api-adapters.test.js` with SHA-256 `4a69f389c931e770ddde7b3f044d1868a2df1bd685a6ce5aada5b9a354c3cfb0`. Domain integration suites separately lock probe output lengths and hashes. Refactors must preserve these baselines; a deliberate gameplay correction needs independent reference evidence and targeted assertions before any affected baseline changes.

`combat-diagnostics.js` composes tank collision, fire limits, fire input, crossing cancellation, and projectile rules in 4+1+1+1+5 probe order. The collision and firing-input bodies live in `combat-tank-collision-diagnostics.js` and `combat-player-fire-input-diagnostics.js`. Their 12 public positions remain 101-112, and their 5,147-byte output hash remains `610833017059b265259ef4b26cc5bed1076687c3e7b181c40588eb6545ad39f2`.

- `tests/run-tests.js` discovers test files in stable path order and runs each in its own Node process; new tests do not need a manual runner entry.
- `tests/unit/` covers pure rules and focused runtime contracts, including AI targeting, collision boundaries, upgrades, dependency validation, receiver binding, and state restoration.
- `tests/helpers/load-browser-scripts.js` loads the production scripts into a VM. `tests/unit/browser-entry.test.js` requires its script order to match `index.html`.
- `tests/helpers/browser-game-harness.js` supplies deterministic DOM, Canvas, audio, storage, and animation-frame doubles. These integration tests do not replace a real-browser visual or audio check.
- `tests/integration/app-bootstrap.test.js` covers startup and toolbar/import wiring. Feature suites cover their own workflows; `tests/integration/editor-render-runtime.test.js` checks the real editor module composition, clearing demo mode on entry, persistent cursor, painting, border rejection, and installing the edited stage on exit.
- `tests/unit/readme-tree.test.js` checks both complete file trees against the workspace, excluding tool metadata directories, checks UTF-8 text and balanced code fences, and compares heading/list/table/fence structure without requiring equal line counts. Translation accuracy still requires review.

### Data And Replacements
...
`data/free-sprite-manifest.json` and `data/free-audio-manifest.json` are mirrored by the frozen browser manifests in `src/presentation/free-sprite-manifest.js` and `src/audio/free-audio-manifest.js`. Tests check manifest parity and independent clones. Rendering and sound generation use these replacement resources rather than original NES graphics or audio.

### Refactor Acceptance

The structural phase delivers a composition-only entry, feature-owned runtime modules, separate domain rules/data, matching unit/integration suites, and bilingual documentation of the complete file structure. Keep these boundaries when adding or correcting mechanics; do not move feature code back into the entry or the public diagnostic composer.

Before a push, run the full suite, check the relevant real-browser workflow, and review both READMEs. The automated suite covers the static script order and public compatibility contracts; it does not prove browser audio quality, every viewport/refresh-rate combination, or a full original-game playthrough.

The next fidelity work must compare the 35-stage campaign and repeat cycles, player/enemy/collision timing, Construction progression, power-ups, result screens, and sound/visual event timing against reference evidence. The modeled random stream still leaves unmodeled NES scratch bytes at their initial values. Replacement artwork/audio and browser additions such as mouse editing, forest outlines, and accessible power-up placement must be tracked explicitly rather than treated as proof of exact NES equivalence.
...
- Brick, steel, water, forest, and ice terrain. Water alternates its two animation frames every 32 gameplay frames. Forest renders as a top cover layer and adds a high-contrast directional tank outline when it covers a tank, while ice draws a projectile-cover layer that makes flying bullets harder to see.
...
- The four centered enemy icons now retain distinct basic, fast, power, and armor silhouettes, including the armor outline and power split barrel.
...
- Per-stage enemy sequences with configurable spawn points and active-enemy limits. The spawn countdown continues at full capacity, including during enemy destruction and the timer power-up, but freezes under Start pause. After expiration, a free slot can refill without another full interval; an early release still waits for the remaining countdown. This follows the `DB48-DB74` spawn handler in the referenced disassembly and is covered in `tests/integration/enemy-spawn-runtime.test.js` for both player modes. A preferred enemy spawn point is tried first; if a tank occupies it, the other two points are tried in fixed order before the spawn enters its retry delay.
...
`enemySpawnRetry` defaults to `25`. If the preferred enemy spawn point is occupied by an active or spawning tank, the other two configured points are tried in fixed order. The pending enemy is not consumed and enters this delay only when all three points are occupied.
