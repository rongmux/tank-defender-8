...
This repository does not include original NES ROM data, original sprites, or original audio. The built-in 35-stage layouts use a fixed, source-derived structural map dataset; map rendering, sprites, and audio use free or custom replacement resources. The built-in enemy composition uses a fixed source-derived 35-stage, 20-enemy-per-stage type order, and the engine remains data-driven so gameplay rules, enemy sequences, and stage packs can be tuned without changing the core code.
...
`src/stages/enemy-sequences.js` stores the fixed type order for all 35 built-in stages, derives consecutive group summaries from that order, and retains the original-style carrier slots and spawn-point rotation. The built-in sequences contain exactly 20 enemies per stage.
...
`tests/helpers/test-file-discovery.js` recursively discovers only `*.test.js` files in stable path order. `tests/run-tests.js` uses it to execute all unit tests, then all integration tests in isolated Node processes, and finally the remaining smoke suite, so new feature tests no longer require a hand-maintained runner entry. Detailed audio, screen-flow, power-up, timer, wall, enemy, and combat probe lifecycle assertions now live in `tests/integration/audio-diagnostics.test.js`, `tests/integration/screen-flow-diagnostics.test.js`, `tests/integration/power-up-diagnostics.test.js`, `tests/integration/timer-diagnostics.test.js`, `tests/integration/wall-diagnostics.test.js`, `tests/integration/enemy-diagnostics.test.js`, and `tests/integration/combat-diagnostics.test.js`; `tools/smoke-test.js` retains only cross-domain regressions that have not yet been migrated.
