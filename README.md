...
`src/runtime/editor-input-runtime.js` owns the fixed-frame Construction input orchestration: cursor movement, original A/B pattern cycling, full-cell and quadrant painting, brush selection, tile cycling, and held-direction repeat. It keeps mutations and sounds as explicit callbacks while `input-runtime.js` owns Canvas coordinate conversion and DOM event wiring. The editor validates and reads `QUAD_GRID` from the stage-grid dependency rather than shared state; out-of-bounds quadrant edits are ignored without terrain changes or paint sounds. Direct tests cover original pattern masks, edge rejection, brush selection, and 20-frame repeat cadence.
...
`src/runtime/input-keyboard-runtime.js` owns keyboard screen dispatch, one-shot fire/stage-selection presses, Arrow/WASD mappings, demo exit, hidden-message input reservation, and editor shortcuts. `src/runtime/input-runtime.js` retains browser event registration, stage-pack file import, Canvas coordinate conversion, and Construction mouse editing; direct tests lock the explicit injection boundary, keyboard routing, and mouse coordinates.
...
`src/runtime/tank-render-runtime.js` owns tank body/tracks, player star-upgrade overlays, shield rendering, player/enemy spawn animation, and the directional outline drawn over forest-covered tanks. It preserves directional geometry, carrier/upgrade color callbacks, pause-safe shield phase, configured spawn durations, and the existing Canvas sprite order; direct tests cover player overlays, exact field offsets, shield colors, player spawn sizing, and forest outlines.
...
`src/runtime/stage-result-render-runtime.js` owns the stage-clear result screen: score headers, one/two-player result rows, type-specific centered tank icons, directional arrows, bonus rows, totals, score formatting, and the closing curtain. It preserves the original 256x240 integer geometry and exposes the result helpers through the compatibility API; direct tests lock the four icon variants, two-player row positions, and center tank/arrow spacing.
...
`src/runtime/editor-render-runtime.js` owns Construction battlefield rendering: the editable terrain layers, base, persistent parent-cell and 8px subcell cursor focus, and six-tile legend. It preserves the original 256x240 field geometry, 16px tile placement, tile masks, and brush highlight while receiving terrain operations through explicit callbacks. Cursor bounds use the validated stage-grid `QUAD_GRID` dependency. Direct tests use the real shared-state constants; `tests/integration/editor-render-runtime.test.js` exercises production module wiring, cursor visibility across palette-cycle frames, keyboard movement/painting, the last quadrant, rejected border clicks, and cursor recovery after leaving the canvas.
...
`src/runtime/audio-bridge.js` owns audio-state setup, mix projection, the composition of extracted audio runtime modules, and the fixed-frame audio lifecycle order exposed to the composition root: updating every voice, stopping gameplay voices before a result, stopping result voices, and stopping every voice on full teardown. Generic Web Audio voice dispatch now lives in `audio-voice-runtime.js`. The unit test locks all four lifecycle sequences so later audio changes cannot silently reorder channel cleanup or advancement.
...
`src/runtime/audio-channel-runtime.js` owns the 17 fixed-frame event channel bindings extracted from `audio-bridge.js`: presentation lookup, audibility selection, node synchronization, start/stop/update lifecycles, and the cross-channel resynchronization needed by pause, stage-start, bonus-life, and power-up events. Its unit test locks the complete channel surface and representative priority callbacks; the audio bridge remains responsible for audio state, mix projection, module composition, and global lifecycle order.

`src/runtime/audio-movement-runtime.js` owns the continuous movement-audio state machine extracted from `audio-bridge.js`: player input requests, demo-mode requests, player/enemy mode resolution, oscillator lifecycle, phase-based frequency updates, and movement-channel synchronization with the audio mix. Its unit test covers input detection, oscillator creation, phase updates, blocking, and teardown.

`src/runtime/audio-voice-runtime.js` owns Web Audio context initialization, sequenced oscillator tracking and cleanup, generic tone creation, manifest voice sequencing, and fixed-frame event redirect routing extracted from `audio-bridge.js`. Its unit test locks sequence timing, generic pitch routing, redirects, and initialization node synchronization.
...
`src/runtime/audio-enemy-hit-lifecycle-diagnostics.js` owns the enemy-hit lifecycle probe extracted from `audio-diagnostics.js`. It preserves armored second-pitch timing, lethal-hit handoff, friendly-fire stun, player-hit destruction, channel priority, and stage cleanup while the composition root restores the original public API position.

`src/runtime/audio-enemy-destroy-diagnostics.js` owns the stateless enemy-destruction audio presentation probe extracted from `audio-diagnostics.js`. Its explicit scope keeps the sampled destruction sequence independent from the enemy release and score lifecycle probe.
...
`src/runtime/audio-power-up-pickup-lifecycle-diagnostics.js` owns the power-up pickup lifecycle probe extracted from `audio-diagnostics.js`. It preserves the 39-frame completion boundary, pause freezing, bonus-life channel masking, and movement-audio restoration while the composition root restores the original public API position.
...
`src/runtime/audio-power-up-appear-lifecycle-diagnostics.js` owns the power-up appearance lifecycle probe extracted from `audio-diagnostics.js`. It preserves carrier release, pause freezing, completion, channel-priority masking, and no-reachable-location cleanup while the composition root restores the original public API position.

`src/runtime/audio-pause-diagnostics.js` owns the stateless pause audio presentation probe extracted from `audio-diagnostics.js`. Its explicit scope keeps the nine phrase-boundary samples independent from pause toggling, frame freezing, and resume lifecycle logic.

`src/runtime/audio-pause-lifecycle-diagnostics.js` owns the pause lifecycle probe extracted from `audio-diagnostics.js`. It preserves pause entry/exit, retained audio frames, final-frame completion, and cleanup behavior while the composition root restores the original public API position.
...
`combat-diagnostics.js` only builds the receiver-preserving combat scope and composes five child modules in tank-collision, fire-limit, fire-input, crossing, and projectile order (4+1+1+1+5 probes). Its unit suite locks input validation, binding precedence, factory call order, direct probe delegation, and shared live state/input/audio references. VM integration checks each probe's owning module and factory registration, executes all 12 probes at their original public indices 101-112, and preserves the pre-refactor 5,147-byte output SHA-256. The complete 159-method public API order is separately protected by its existing SHA-256 regression test.
...
`enemy-spawn-runtime.js` owns the live enemy creation boundary extracted from `src/game.js`: active-slot capacity, preferred and fallback spawn-point checks, retry timing when all points are blocked, carrier cleanup, enemy construction, and player-scaled fixed-frame spawn pacing. Its unit test locks the state.fn registration, sequence limits, explicit delays, default pacing, occupied-point fallback, all-points retry, and carrier callback; existing browser enemy diagnostics continue to verify the real stage flow.
...
`src/runtime/input-keyboard-runtime.js` owns keyboard routing outside the fixed-frame simulation. `src/runtime/input-runtime.js` delegates that frozen handler pair through an explicit composition dependency while retaining stage-pack imports and Construction mouse editing; its tests lock the input contract without duplicating gameplay rules.
