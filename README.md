...
- Game over: the in-field banner completes before input is accepted. On the following full-screen `GAME OVER` interstitial, `Enter` acts as Start and `Escape` acts as Select; either skips the remaining fanfare.
...
- Stage intro, original-timed per-stage result count-up table, in-field game-over banner, and the separate black full-screen `GAME OVER` presentation. The full-screen lettering starts at the original coordinates, advances each replacement glyph by 32 pixels, waits for a three-second free three-voice procedural fanfare, and can be skipped only there with keyboard Start/Select. Score, persistent high score with the original 20000-point floor, reserve-life display, reserve enemy counter, and pause are also implemented.
