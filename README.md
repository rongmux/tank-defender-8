...
Packs may set `gameSettings.projectileRules`. `bulletSize` is the bullet collision/render size in pixels, `spawnOffset` is the muzzle offset from the tank center, and `boundsPadding` is the off-field margin before a bullet is removed. Enemy and friendly bullets hit a player only when both center-coordinate differences are below `10` pixels; a helmet or post-spawn force field absorbs the bullet without a hit explosion. If omitted, the defaults are `4`, `9`, and `4`; valid ranges are `bulletSize` `1` through `16`, `spawnOffset` `0` through `32`, and `boundsPadding` `0` through `32`.
