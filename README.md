...
`tests/helpers/test-file-discovery.js` recursively discovers only `*.test.js` files in stable path order. `tests/run-tests.js` uses it to execute all unit tests, then all integration tests in isolated Node processes, and finally the remaining smoke suite, so new feature tests no longer require a hand-maintained runner entry. Detailed audio and screen-flow probe lifecycle assertions now live in `tests/integration/audio-diagnostics.test.js` and `tests/integration/screen-flow-diagnostics.test.js`; `tools/smoke-test.js` retains only cross-domain regressions that have not yet been migrated.
