...
- 单人和双人模式。
- 原版风格标题演示会在标题画面待机 2560 帧后开始（按固定 60 Hz 逻辑速率约为 42.7 秒）。演示会直接启用两名 AI 玩家，使用当前免费替代关卡数据并显示第 30 关，维持四个活动敌人的上限，优先追踪可用道具，并在不增加分数或结算表击杀数的情况下应用战斗效果。进入过建造模式后，后续自动演示会停用，直到重置游戏。
