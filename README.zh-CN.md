...
- 战斗移动现已加入免费的程序化替代音频，并遵循原版声道行为：活动战斗期间持续播放敌方双音高引擎循环；只要玩家按住方向键，玩家移动声就会取得优先级，即使坦克被阻挡，或仍处于非零的死亡/出生状态。暂停、游戏结束和通关后的战场延迟都会静音这两个循环。每次开始新的冰面滑行时，还会独立触发一个四音、持续 52 帧的提示声。跨过关卡包配置的奖励生命分数阈值，或拾取额外坦克道具时，都会触发同一组双脉冲波替代号角：两个声部分别持续 60 和 54 个固定逻辑帧，暂停会将二者静音并冻结，第二声部在第 54 帧结束时恢复移动脉冲声道。
