...
- 砖墙、钢墙、水域、树林和冰面地形。水面每 32 个游戏帧切换一次两种动画画面。树林作为顶层遮挡渲染，覆盖坦克时会补绘高对比度的方向相关坦克轮廓；冰面会绘制弹丸遮挡层，使飞行中的子弹更难看清。
...
- 关卡结算页的四种居中敌方图标保留基础、快速、火力和装甲四种不同轮廓，其中装甲图标带装甲边框、火力图标带分叉炮管。
...
- 每关独立的敌人序列，并可配置出生点和同时活动的敌人数上限。满员时生成倒计时仍继续递减，包括敌人爆炸和时钟道具生效期间，但 Start 暂停会冻结倒计时。倒计时结束后，槽位释放即可补充敌人，无需再等待完整间隔；提前释放则仍需等待剩余倒计时。这与参考反汇编的 `DB48-DB74` 生成逻辑一致，`tests/integration/enemy-spawn-runtime.test.js` 覆盖了单人和双人模式。敌人会优先尝试指定出生点；如果该位置被坦克占用，则按固定顺序尝试另外两个出生点，只有三个点都被占用时才进入重试延迟。
...
`data/free-audio-manifest.json` 定义当前免费/自定义程序化音效集。运行时通过 `window.TankDefender8.audioManifest()` 公开相同数据，`tests/integration/free-audio-manifest.test.js` 会检查文件与运行时副本是否保持同步。
...
`enemySpawnRetry` 默认为 `25`。如果首选敌军出生点被活动中或正在出生的坦克占用，会按固定顺序尝试另外两个配置出生点。只有三个出生点都被占用时，待生成的敌军才不会被消耗并进入该延迟。
...
每一关也可以设置 `powerUpSpawns`，即携带者释放道具时使用的固定候选位置。这些位置同样使用 13x13 图块坐标，并且必须至少包含一个点。省略时，引擎使用 X/Y 坐标轴均为 `2`、`5`、`8`、`11` 的原版风格 4x4 笛卡尔积列表。对于这个内置列表，运行时分别为 X 和 Y 消耗一个随机字节，使用各自的低 2 位，然后再消耗下一个随机字节的低 3 位查固定的 8 项道具表。放置前仍会过滤被阻挡或不可到达的候选点；如果过滤减少了原始列表，就把同一个 4x4 抽样索引映射到剩余候选点。自定义列表仍使用通用的 16 位抽样，并避开刚刚使用过的道具位置。
...
内置关卡包现使用固定的 35 关公开源码推导结构化地图数据，以及来自公开 Battle City（日版）反汇编、并展开为每关 20 辆敌人的固定敌人编组顺序与数量表。`data/free-35-stage-pack.json` 仍为全部 35 关提供固定的免费/自定义替代地图，`data/free-audio-manifest.json` 提供当前程序化替代音效事件，`data/free-sprite-manifest.json` 提供当前程序化坦克、子弹、地形、基地、普通/摧毁爆炸、面板、轮廓和道具精灵。未来的采样音乐/音效或更丰富的精灵图同样应使用免费/自定义替代资源，而非源自原版 ROM 的资源。用于交叉核对规则的公开参考资料包括：
