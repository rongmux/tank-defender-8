...
`tests/helpers/test-file-discovery.js` 按稳定路径顺序递归发现 `*.test.js` 文件。`tests/run-tests.js` 使用它依次在隔离的 Node 进程中运行全部单元测试、全部集成测试，最后运行剩余 smoke 套件，因此新增功能测试不再需要手工维护运行器清单。音频与屏幕流程探针的详细生命周期断言现在分别由 `tests/integration/audio-diagnostics.test.js` 和 `tests/integration/screen-flow-diagnostics.test.js` 负责；`tools/smoke-test.js` 只保留尚未迁移的跨域回归。
