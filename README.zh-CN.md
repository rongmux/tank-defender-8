...
- 战斗移动现已加入免费的程序化替代音频，并遵循原版声道行为：活动战斗期间持续播放敌方双音高引擎循环；只要玩家按住方向键，玩家移动声就会取得优先级，即使坦克被阻挡，或仍处于非零的死亡/出生状态。暂停、游戏结束和通关后的战场延迟都会静音这两个循环。每次开始新的冰面滑行时，还会独立触发一个四音、持续 52 帧的提示声。
